...
import { updatePersonalAdmin, getFacultyData } from '../../../actions/actionsAdmin';
...
import FileBase from 'react-file-base64';
    ...
    const initialState = {email: '', password: '', picture: '', firstName: '', middleName: '', nameExtension: '', lastName: '', dateOfBirth: '', age: '', placeOfBirth: '', gender: '', civilStatus: '', mobile: '',
    weight: '', height: '', bloodType: '', altEmail: '', telephone: '', citizenship: '', rHouseBlockNo: '', rStreet: '', rSubdivisionVillage: '',
    rBarangay: '', rCityMunicipality: '', rProvince: '', rZipCode: '', pHouseBlockNo: '', pStreet: '', pSubdivisionVillage: '',
 ...
 const UpdatePersonal= (props) =>  {
  ...
  const [postData, setPostData] = useState({acc});
  ...
  const handleSubmit = async (e) => { 
    ...
    dispatch(updatePersonalAdmin(id, postData));
            ...
            <Stack direction='column' spacing={3} justifyContent='center'>
              ...
              <Stack direction="row" alignItems="center" justifyContent='center' spacing={2}>
                ...
                <Typography variant="h5" sx={{ fontWeight:300, color: "#14213D" }}>Choose a Profile Picture</Typography>
                 <FileBase type="file" multiple={false} onDone={({base64}) => setPostData({...postData, picture: base64})}/>
                </label>
                <Divider />
              </Stack>
              <Stack justifyContent='center' direction='row' spacing={1}>
                <TextField id="outlined-required" variant='outlined' onChange={handleChange} name ='email' type='email' label="Email" sx={longInputStyle}/>
                <TextField id="outlined-required" variant='outlined' onChange={handleChange} name ='password' type='password' label="Password" sx={shortInputStyle} />
              </Stack>
              <Divider />
              <Stack justifyContent='center' direction='row' spacing={1}>
